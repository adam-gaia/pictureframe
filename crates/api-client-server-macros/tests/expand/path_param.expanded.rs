...
impl MyApp {
    pub async fn get_user(&self, id: UserId) -> MyResult<User> {
        Ok(User {
            id,
...
impl MyApp {
    ...
    pub fn router(self: std::sync::Arc<Self>) -> axum::Router {
        axum::Router::new()
            .route("/users/{id}", axum::routing::get(__axum_handler_get_user))
            .with_state(self)
    }
}
async fn __axum_handler_get_user(
...
impl MyAppClient {
    ...
    pub async fn get_user(&self, id: UserId) -> Result<User, MyAppClientError> {
